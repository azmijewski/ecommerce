...
public interface OrderRepository extends PagingAndSortingRepository<Order, Long> {
    ...
    List<Long> getExpiredOrdersIdWhereWaitingForPayment(@Param("date") OffsetDateTime date);

    @Query(value = "call get_orders_waiting_for_payment_to_expire_in(:date, :days)", nativeQuery = true)
    List<Order> getOrdersToExpiredInDays(@Param("date") OffsetDateTime date, @Param("days") Integer days);
}
